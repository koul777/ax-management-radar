  ...
  return (
    <main className="app-main">
      <header className="comparison-menu-shell ax-app-shell">
        ...
        <div className="comparison-menu ax-main-menu" role="tablist" aria-label="자료영역 선택">
          {menuGroups.map((group) => <section key={group.title} className="ax-menu-group" aria-label={group.title}><strong>{group.title}</strong><div>{group.items.map((menu) => <button id={`menu-${menu.id}`} aria-controls="dashboard-content" aria-selected={activeMenu === menu.id} className={activeMenu === menu.id ? "active" : ""} key={menu.id} onClick={() => selectMenu(menu.id)} role="tab" type="button"><span>{menu.title}</span><small>{menu.subtitle}</small></button>)}</div></section>)}
        </div>
