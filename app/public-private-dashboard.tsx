...
const insights: Record<string, string> = {
  proactivity: "표시한 다섯 요인 중 변화주도성의 표준화 계수가 두 집단 각각에서 가장 큽니다. 자기보고 문항 내용이 가까운 영향도 있을 수 있어, 인과적 동력이나 관리 개입의 효과 순위로 읽지는 않습니다.",
  autonomy: "두 집단 각각에서 양의 연관성이 관찰됐고, 공공의 평균이 더 낮습니다. 권한선과 실험 허용범위 정비는 이 현황과 선행연구를 바탕으로 현장 시범운영에서 검증할 후보 과제입니다.",
...
const literature: Record<string, ComparisonFactor["literature"]> = {
  ...
  public_service_motivation: {
    ...
    caution: "선행연구 결과가 완전히 같지 않아 2024 자료의 명시된 통제모형 결과와 구분해 읽습니다.",
  },
...
const config: ComparisonDashboardConfig = {
  ...
  title: "공공·민간 혁신행동 연관성 비교",
  question: "어떤 요인이 혁신행동과 실제로 연결되고, 두 집단은 어디에서 다른가?",
  headline: "변화주도성은 두 집단 모형에서 큰 양의 조정계수와 p<.05 연관을 보였습니다. 다만 공공·민간 직접 계수차의 재현 검증은 미완료입니다. 공공의 리더십 4유형 평균은 모두 민간보다 낮지만, 15문항 통합 리더십의 조정 후 연관은 두 집단에서 p<.05가 아니었습니다.",
  scopeBanner: "해석 범위: 이 메뉴는 원고와 종속변수가 다른 2024 공공·민간 자료입니다. 논문 결과를 그대로 복제한 화면이 아닙니다. 기존 집계는 유지하되 생성 코드·가중치 처리·표준화 상수의 재현 검증은 미완료입니다.",
  plainSummary: [
    {
      label: "집단별 조정 후 연관",
      title: "두 집단 모형에서 변화주도성과 공공봉사동기의 양의 계수가 컸습니다.",
      body: "스스로 문제를 찾고 끝까지 밀고 가는 성향, 그리고 자신의 일이 사회에 도움이 된다고 느끼는 정도가 혁신행동과 조정 후 연관되었습니다. 통합 리더십은 공공 β=.102(p=.051), 민간 β=.005(p=.915)로 p<.05 기준의 근거가 부족했습니다. 효과가 없다는 증명은 아닙니다. 부문이 같다는 증명도 아닙니다.",
    },
    {
      label: "현재 수준 차이",
      title: "이 응답표본에서 공공의 자율성·공정성 체감은 더 낮았습니다.",
    ...
    },
    {
      ...
      body: "공공은 윤리적 −0.09점, 서번트 −0.25점, 카리스마 −0.29점, 포용적 리더십 −0.13점으로 모두 낮았습니다. 현재 수준을 점검할 신호입니다. 통합 리더십의 비유의성만으로 네 유형 각각의 효과 유무를 판정할 수 없으며, 다른 요인과 함께 시범운영에서 확인해야 합니다.",
    },
  ],
  ...
  controlTermsLabel: `${data.meta.control_terms}개 더미·연속항: 기존 집계에 기재된 통제항`,
  ...
  changeManagement: {
    ...
    description: "민간과의 현황 격차 및 통제 후 연관성을 바탕으로, 90일 시범운영에서 검증할 후보 과제를 제안합니다. 통계로 최적 처방을 확정한 것은 아닙니다.",
    audiences: [
      {
        id: "public-manager",
        label: "공공조직 관리자",
        diagnosis: {
          title: "실행 조건을 우선 점검할 가설이 있습니다.",
          body: "저장된 집계에서 공공 구성원의 혁신행동 평균은 민간과 비슷했습니다. 자율성과 공정성 체감, 만족이 낮고 업무부담은 높게 관찰됐지만, 재현 미완료 상태이므로 원인이나 개입 우선순위로 확정하지 않습니다.",
          signals: [
            { label: "혁신행동", value: "공공 3.57 · 민간 3.54", note: "평균 차이 p=.384 · 뚜렷한 차이 없음" },
            { label: "업무 자율성", value: "공공 3.14 · 민간 3.55", note: "공공 −0.40 · 저장된 모형의 p<.05 연관과 현황 차이 · 재현 미완료" },
            { label: "조직공정성", value: "공공 2.67 · 민간 3.26", note: "공공 −0.59 · 직접 처방보다 현황 위험신호" },
          ],
        },
        priority: {
          eyebrow: "조정 후 연관 + 개선 여지 + 현업 검증",
          title: "현장 개선과제부터 자율적 실행 범위를 먼저 명시하세요.",
          evidence: "공공 모형에서 업무 자율성 β=.145(p=.001), 변화주도성 β=.427(p<.001)의 양의 조정 후 연관이 관찰됐습니다. 생성 코드·가중치·표준화 상수의 재현 검증과 공공·민간 직접 대비는 미완료입니다.",
        ...
        roadmap: [
          ...
          { period: "61~90일", title: "유지·중단·확산을 결정합니다", body: "기존 방식과 처리시간·민원경험을 비교하고, 사전에 정한 개선·위험 기준을 충족한 방식만 다음 시범 후보로 남깁니다." },
        ],
      },
    ],
    decisionRule: "관찰된 연관성 + 현재 수준 + 현업 조정 가능성 → 시범운영 후보 선정",
    caution: "집단 평균 격차를 개인 평가로 옮기지 않습니다. 가능하면 비교부서와 사전 추세를 함께 기록하고, 단순 개입 전후 차이만으로 인과효과를 확정하지 않습니다.",
  },
  differenceStory: {
    ...
    steps: [
      ...
      },
      {
        ...
        title: "두 집단 각각에서 양의 연관성이 관찰된 요인이 있습니다.",
        body: "변화주도성, 업무 자율성, 공공봉사동기는 명시된 통제변수를 넣은 뒤 두 집단 각각에서 혁신행동과 연결됐습니다. 계수의 동일성이나 공통 인과 원리를 입증한 결과는 아닙니다.",
        signals: [
      ...
      },
      {
        ...
        body: "공공은 자율성과 평가 공정성 체감이 낮고 업무부담은 높았습니다. 권한선·평가 설명·실험시간 정비를 현장 검증용 후보 과제로 삼을 수 있습니다. 어느 조치가 혁신을 더 높이는지는 별도 평가가 필요합니다.",
        signals: [
    ...
    conclusion: "저장된 기술·회귀 집계에서는 공공의 아이디어 실행 여건 차이가 눈에 띄지만, 재현 미완료 상태이므로 공공·민간 차이나 관리 개입의 효과로 확정하지 않습니다.",
  ...
  reproducibilityNote: "위 순서는 기존 화면·집계의 설명입니다. 생성 코드가 추적되지 않아 전체표본 표준화의 대상·상수, WT의 실제 처리 및 표본 선택을 이번 검토에서 재현하지 못했습니다. 집단별 표준화를 사용했다고 바꾸어 단정하지도 않습니다.",
  warnings: [
    "2026-09-07 검토에서 이 JSON의 생성기를 찾지 못했습니다. 표본 제외·WT 처리·표준화 상수·직접 대비 산출식의 재현은 미확인이며, 저장된 difference_p를 검증된 공공·민간 차이로 표시하지 않습니다.",
