...
const insights: Record<string, string> = {
  goal_clarity: "모든 통제변수를 반영한 뒤에는 중앙과 지방 모두 유효하지 않았습니다. 목표를 명확히 하는 것만으로 혁신행동이 높아진다고 단정할 수 없습니다.",
  ...
  training: "지방에서는 유효했고 중앙에서는 p=.058로 기준에 조금 못 미쳤습니다. 지방에서는 강의 수료보다 현장 적용형 교육을 먼저 시험할 근거가 됩니다.",
  participation_communication: "지방에서는 유효하고 중앙에서는 유효하지 않았습니다. 지방 현장에서는 의견을 받고 답하는 구조를 우선 시험할 실행 단서입니다.",
  public_service_motivation: "중앙과 지방 모두에서 가장 큰 유효 요인입니다. 공공가치를 시민이 체감하는 업무 개선 과제와 연결하는 것이 공통 우선순위입니다.",
};
...
const config: ComparisonDashboardConfig = {
  ...
  title: "중앙·지방 혁신행동 영향 대시보드",
  question: "같은 변화관리 요인이 중앙정부와 지방자치단체에서 어떻게 다르게 작동하는가?",
  headline: "전체 6,084명 통합모형에서는 공공봉사동기·교육훈련·참여 및 소통이 유효했습니다. 집단별로는 공공봉사동기만 중앙과 지방 모두에서 확인됐습니다.",
  plainSummary: [
    {
      label: "확실히 확인된 것",
      title: "전체를 합쳐도 공공봉사동기가 가장 분명했습니다.",
      body: "통합모형에서 공공봉사동기(β=.225), 교육훈련(β=.055), 참여·소통(β=.044)이 유효했습니다. 집단별로 나누면 공공봉사동기만 중앙과 지방 모두에서 확인됐습니다.",
    },
    {
      label: "현재 수준 차이",
    ...
    },
    {
      ...
      body: "지방은 공공봉사동기 0.17점, 업무수행역량 0.14점, 업무태도 0.12점이 중앙보다 낮았습니다. 교육훈련과 참여·소통은 지방 내부에서 혁신행동과 연결돼 우선 시험할 관리 경로입니다.",
    },
  ],
  ...
  changeManagement: {
    ...
    description: "공통 방향과 기관유형별 실행순서를 나눠, 연구결과를 시민이 체감하는 90일 개선과제로 바꿉니다.",
    audiences: [
      {
        id: "common",
        label: "공통 방향",
        diagnosis: {
          title: "공공가치를 실제 개선으로 바꾸는 연결고리가 핵심입니다.",
          body: "중앙과 지방 모두 공공봉사동기가 혁신행동과 가장 강하게 연결됐습니다. 동시에 지방의 공공봉사동기·업무수행역량·업무태도 수준은 중앙보다 낮아 실행기반을 함께 보완할 필요가 있습니다.",
          signals: [
            { label: "공공봉사동기 영향", value: "중앙 β=.232 · 지방 β=.224", note: "두 집단 모두 p<.001 · 가장 큰 공통 유효 요인" },
        ...
        priority: {
          ...
          evidence: "공공봉사동기는 중앙 β=.232, 지방 β=.224로 모든 통제변수를 반영한 뒤에도 두 집단에서 유의했습니다.",
          ...
          nextMoves: [
            ...
            "한 집단에서만 유의한 결과는 기관유형 고유효과로 단정하지 않습니다.",
          ],
        },
        metrics: [
        ...
        roadmap: [
          { period: "0~30일", title: "시민 문제를 하나 고릅니다", body: "민원·이용자 경험·현장자료에서 반복되는 불편을 정하고 현재 기준값을 저장합니다." },
          { period: "31~60일", title: "안전한 작은 시험을 엽니다", body: "기간·예산상한·책임자·중단조건을 명시해 한 부서에서 먼저 실행합니다." },
          { period: "61~90일", title: "체감결과로 확산을 결정합니다", body: "과정·인식·결과지표를 함께 검토해 유지·수정·중단·확산 중 하나를 결정합니다." },
        ],
      },
      {
        id: "central",
      ...
      },
      {
        ...
        diagnosis: {
          ...
          body: "지방에서는 공공봉사동기·교육훈련·참여 및 소통이 혁신행동과 연결됐습니다. 이를 주민 현안형 학습과 반드시 답하는 제안 절차로 시험할 수 있습니다.",
          signals: [
            { label: "공공봉사동기", value: "β=.224 · p<.001", note: "지방에서도 가장 큰 유효 요인 · 현재값 3.09" },
            { label: "교육훈련", value: "β=.055 · p=.009", note: "교육기회 2.99 · 현장적용 경로를 우선 확인" },
            { label: "참여·소통", value: "β=.054 · p=.021", note: "자유로운 개선 건의 2.90 · 회신 구조 필요" },
          ],
        },
        priority: {
          eyebrow: "지방 내부에서 확인된 보완 레버",
    ...
    decisionRule: "공통 유효요인은 기본방향으로, 집단별 현재 수준과 내부 유효요인은 실행순서를 정하는 단서로 사용",
    caution: "기관유형별 제안은 실행 우선순위이지 효과 차이의 증명은 아닙니다. 부서 단위 시범운영과 동일 문항의 도입 전후 추적으로 다시 확인하십시오.",
  },
  differenceStory: {
    title: "중앙과 지방의 혁신 차이는 어디서 보이나?",
    description: "혁신행동 격차를 공통 동력과 집단별 실행기반으로 나눠 읽습니다.",
    shortValue: "0.14점",
    ...
    steps: [
      ...
      {
        eyebrow: "공통 동력부터 확인",
        title: "두 집단 모두 공공봉사동기가 핵심입니다.",
        body: "국민과 사회에 도움이 되는 일을 중요하게 여길수록 혁신행동이 높았습니다. 중앙과 지방 모두에서 다른 요인보다 가장 큰 관계가 확인됐습니다.",
        signals: [
          { label: "공공봉사동기 영향", value: "β .232 · .224", note: "중앙 · 지방 모두 p<.001" },
      ...
      },
      {
        ...
        signals: [
          ...
          { label: "지방 내부 실행 단서", value: "교육·참여 유효", note: "교육 p=.009 · 참여·소통 p=.021" },
        ],
      },
    ],
  ...
  overallRegression: {
    ...
    description: "두 집단을 모두 합친 뒤 기관유형과 원고의 개인·조직 조건을 통제한 공통 영향요인",
    ...
    caution: "통합 결과는 전체 공직사회의 평균적 관계입니다. 중앙과 지방의 실행 우선순위는 바로 아래 집단별 결과를 함께 보고 정해야 합니다.",
  },
  ...
  culture: {
    ...
    literature: {
      ...
      why: "조직문화는 구성원에게 어떤 행동이 자연스럽고 보상받는지를 알려주는 공통의 환경이므로, 핵심요인의 영향과 섞이지 않도록 통제하면서 두 조직의 문화 프로필도 따로 비교했습니다.",
